...
- `political_score_threshold` is applied to `topic.News & Social Concern`.
...
`posts_core_*.parquet` includes nullable `is_political`: sampled negatives meeting the `topic.News & Social Concern` threshold are `true`; all other sampled negatives and liked-only rows are null.

...
Stage 7 loads the narrow selected-URI lookup once, then scans raw post and reply files once in bounded batches controlled by `embedding_source_batch_size` (default `64`). It processes independent source batches concurrently with shared lookup state, and later validates and decodes independent URI partitions in separate worker processes. Both phases use `dataset_hydration_worker_count` (default `4`) as their concurrency budget, but their pools never overlap. Each winning decoded vector is retained so it can be written without a second decode. Set the worker count to `1` for the lower-memory serial path. The final memmap is assembled deterministically in URI-partition order.

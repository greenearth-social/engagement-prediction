...
01_get_data -> 02_target_posts -> 03_user_history -> 04_train -> 05_evaluate
...
def get_stage_input_folders() -> Dict[str, List[str]]:
    ...
    For example, ``04_train`` depends on ``01_get_data``,
    ``02_target_posts``, and ``03_user_history``. The result is derived from
...
def validate_explicit_prior_pin_consistency(ctx: Context) -> None:
    ...
    ``02_target_posts`` artifact whose manifest says it was built from a

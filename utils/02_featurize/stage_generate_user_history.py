...
Creates a directory-style artifact that maps each target (did, post_id) to a list
of prior liked post embedding indices, enabling efficient on-the-fly embedding
retrieval during training.

Inputs:
- likes_core_*.parquet from 01_get_data: Contains {did, subject_uri, record_created_at, emb_idx}
- target_posts_*.parquet from prior stage: Contains {did, post_id, liked, ts, emb_idx, split}

Outputs under <run_dir>/02_featurize/<timestamp>/:
- user_history_directory_<timestamp>.parquet: {target_idx, did, post_id, prior_emb_indices}
...
def _build_user_history_directory(
    ...
    Args:
        targets_lf: LazyFrame with columns [did, post_id, ts, ...]
    ...
    Returns:
        LazyFrame with columns [did, post_id, prior_emb_indices]
    """
    ...
    # Targets without prior likes will have null prior_emb_indices, which we convert to empty list
...
def run(context: Context, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    Creates a parquet file mapping each target (did, post_id) to a list of
    prior liked post embedding indices for efficient on-the-fly lookup.
    """
    ...
    # 2. Target posts from prior stage (expected to be passed via prior_outputs or found automatically)
    # For now, look for target_posts_*.parquet in the same get_data output or a dedicated stage
    # The user indicated this comes from a prior stage
    target_posts_path = context.prior_outputs.get('target_posts')
    if target_posts_path:
        target_posts_path = Path(target_posts_path)
    else:
        # Try to find in get_data output
        target_posts_candidates = list(prior_get_data.glob("target_posts_*.parquet"))
        if not target_posts_candidates:
            raise FileNotFoundError(
                f"No target_posts_*.parquet found. Provide via --prior-output-target-posts "
                f"or ensure prior stage creates it."
            )
    ...
    output_path = out_dir / f"user_history_directory_{out_dir.name}.parquet"
    
    # Add target_idx as row index for reference
    directory_df = directory_lf.collect()
    directory_df = directory_df.with_row_index("target_idx")
    
    # Ensure column order: target_idx, did, post_id, prior_emb_indices
    directory_df = directory_df.select(["target_idx", "did", "post_id", "prior_emb_indices"])
